...
public interface StatRepository extends JpaRepository<EndpointHit, Integer> {
    ...
    @Query("select new ru.practicum.model.ViewStats(e.app, e.uri, count(distinct e.ip) as hits) from EndpointHit as e where e.uri IN :uris AND e.timestamp " +
    ...
    @Query("select new ru.practicum.model.ViewStats(e.app, e.uri, count(e.uri) as hits) from EndpointHit as e where e.uri IN :uris AND e.timestamp " +
            "BETWEEN :start AND :end group by e.uri, e.app order by hits desc")
    ...
    @Query("select new ru.practicum.model.ViewStats(e.app, e.uri, count(e.uri) as hits) from EndpointHit as e where e.timestamp " +
            "BETWEEN :start AND :end group by e.uri, e.app order by hits desc")
    public List<ViewStats> findRequestWithNullUris(@Param("start") LocalDateTime start, @Param("end") LocalDateTime end);

    @Query("select distinct new ru.practicum.model.ViewStats(e.app, e.uri, count(distinct e.uri) as hits) from EndpointHit as e where e.timestamp " +
            "BETWEEN :start AND :end group by e.uri, e.app order by hits desc")
